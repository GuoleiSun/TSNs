...
import cv2
from model import ResNetUNet2_2
...
def main(flags):
    ...
    n_ones=20
    for task in task_sets:
        z = torch.zeros([1, 5*n_ones], dtype=torch.float)
        if task == 'seg':
            z[:, :n_ones] = 1
        elif task == 'parts':
            z[:, n_ones:2*n_ones] = 1
        elif task == 'edges':
            z[:, 2*n_ones:3*n_ones] = 1
        elif task == 'normals':
            z[:, 3*n_ones:4*n_ones] = 1
        elif task == 'saliency':
            z[:, 4*n_ones:] = 1
        latent_z_task[task] = z
    ...
    # model = ResNetUNet2_2(numclass).to(device)
    model = ResNetUNet2_2(numclass, dim_latent=5*n_ones).to(device)
    ...
    # print("**************saving condition weights**************")
    # condition_weights=[model.fcs(latent_z_task[task_sets[i]].cuda()) for i in range(len(task_sets))]
    # # condition_weights=[i.cpu().numpy()  for i in condition_weights]
    # condition_weights=[i.detach().cpu().numpy()  for i in condition_weights]
    # condition_weights=np.array(condition_weights)
    # condition_weights=condition_weights.squeeze()
    # print(condition_weights.shape)
    # np.save("condition_weights-iiai_5tasks_139_n_ones_50", condition_weights)
    # exit()
