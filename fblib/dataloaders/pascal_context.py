...
class PASCALContext(data.Dataset):
    ...
    def __init__(self,
        ...
        if self.do_semseg or do_human_parts or do_edge or do_sal:
    ...
    def __getitem__(self, index):
        ...
        # added by guolei to use student's augumentation for seg training
        if self.do_semseg and self.split[0]=='train':
            ...
            sample['semseg'] = _semseg
            return sample
        if self.do_human_parts and self.split[0]=='train':
            # print("do_human_parts")
            _img = cv2.imread(self.images[index])
            _img = cv2.cvtColor(_img, cv2.COLOR_BGR2RGB)
            _img = cv2.resize(_img, (IMG_SIZE[1], IMG_SIZE[0]),interpolation=cv2.INTER_CUBIC)
            _human_parts, _ = self._load_human_parts(index)
            _human_parts = cv2.resize(_human_parts, (IMG_SIZE[1], IMG_SIZE[0]), interpolation=cv2.INTER_NEAREST)
            img_mask = self.seg_aug_student(image=_img, mask=_human_parts)
            _img = img_mask['image']
            _human_parts = img_mask['mask']
            _img = self.trfinput(_img)
            _human_parts = self.trftarget(_human_parts)
            sample['image'] = _img
            sample['human_parts'] = _human_parts
            return sample
        if self.do_sal and self.split[0]=='train':
            # print("do_sal")
            _img = cv2.imread(self.images[index])
            _img = cv2.cvtColor(_img, cv2.COLOR_BGR2RGB)
            _img = cv2.resize(_img, (IMG_SIZE[1], IMG_SIZE[0]),interpolation=cv2.INTER_CUBIC)
            _sal = self._load_sal_distilled(index)
            _sal = cv2.resize(_sal, (IMG_SIZE[1], IMG_SIZE[0]), interpolation=cv2.INTER_NEAREST)
            img_mask = self.seg_aug_student(image=_img, mask=_sal)
            _img = img_mask['image']
            _sal = img_mask['mask']
            _img = self.trfinput(_img)
            _sal = self.trftarget(_sal)
            sample['image'] = _img
            sample['sal'] = _sal
            return sample
        ...
        # print("here1: ", sample['image'].max(), sample['image'].min())
    ...
    def _load_img(self, index):
        ...
        # print("here1111: ", _img.max(), _img.min(), _img.dtype)
        # exit()
        return _img
...
class PASCALContext_old(data.Dataset):
    """
    PASCAL-Context dataset, for multiple tasks
    Included tasks:
        1. Edge detection,
